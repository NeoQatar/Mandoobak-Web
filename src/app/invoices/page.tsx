...
export default function InvoicesPage() {
  ...
  const [invoicesRaw, setInvoices] = useState<InvoiceWithOrderDetails[]>([]);
  const invoices = useTranslatedData(invoicesRaw);
  ...
  const fetchInvoices = async () => {
    ...
    const vendorFilteredInvoices = isVendor && vendorId
        ? data.filter(inv => inv.vendorId === vendorId)
        : isVendor ? [] : data;

    const sortedInvoices = vendorFilteredInvoices.sort((a, b) => {
                ...
                <TableRow>
                  <TableCell colSpan={7} className="h-64 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="text-muted-foreground text-sm font-medium animate-pulse">Loading invoices...</span>
                      </div>
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <TableCell>
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${
                          invoice.orderStatus === 'Order Completed'
                            ? 'bg-green-100 text-green-800'
                            : invoice.orderStatus === 'Cancelled'
                            ? 'bg-red-100 text-red-800'
                            : invoice.orderStatus === 'Order Inprogress'
